...
// Full AES-256 implementation (Non-mock)
...
static const uint8_t rsbox[256] = {
  ...
  0x72, 0xf8, 0xf6, 0x64, 0x86, 0x68, 0x98, 0x16, 0xd4, a4, 0x5c, 0xcc, 0x5d, 0x65, 0xb6, 0x92,
...
#define Multiply(x, y) (((y & 1) * x) ^ ((y >> 1 & 1) * (x << 1 ^ (x >> 7 ? 0x1b : 0))) ^ ((y >> 2 & 1) * (x << 2 ^ (x << 1 >> 7 ? 0x1b : 0) ^ (x >> 6 ? 0x36 : 0))))
...
static void ShiftRows(uint8_t (*state)[4]) {
    ...
    temp = state[0][3]; state[0][3] = state[3][3]; state[3][3] = state[2][3]; state[2][3] = state[1][3];
}
...
void KeyExpansion(const uint8_t* Key, uint8_t* RoundKey) {
    ...
    for (i = 8; i < 60; ++i) {
        ...
        if (i % 8 == 0) {
            uint8_t u8tmp = tempa[0]; tempa[0] = tempa[1]; tempa[1] = tempa[2]; tempa[2] = tempa[3]; tempa[3] = u8tmp;
